...
st.markdown("Diagnostic & descriptive intelligence — trend detection, mode analysis, decomposition, and envelope mapping.")
...
def get_snapshot_datetime(snapshot):
    """Return a datetime for the snapshot using hardcoded dates first, then sequential fallback."""
    # 1. Hardcoded dates (most reliable)
    hardcoded = {
    ...
    # 2. Try to get from demo DataFrame (if it has a Date column)
    try:
    ...
    # 3. Sequential fallback based on snapshot order
...
# ============================================================
# SECTION 1: PERFORMANCE DEGRADATION TREND
# ============================================================
st.header("📉 1. Performance Degradation Trend")
st.markdown("Tracks key KPIs (Heat Rate, Output, Efficiency) over time with a linear trend line.")

trend_data = []
for snap in snapshots:
    dt_val = get_snapshot_datetime(snap)
    if dt_val is None:
        continue
...
if len(trend_data) < 2:
    ...
    with col1:
        st.metric("HR Degradation Slope", f"{slope_hr:.2f} {unit}/point",
                  delta=f"{slope_hr * len(df_trend):.1f} {unit} over period",
                  delta_color="inverse" if slope_hr > 0 else "normal")
    with col2:
        coeffs_out = np.polyfit(x_num, df_trend["Output (MW)"], 1)
        slope_out = coeffs_out[0]
        st.metric("Output Degradation Slope", f"{slope_out:.3f} MW/point",
                  delta=f"{slope_out * len(df_trend):.2f} MW over period",
    ...
    fig.add_trace(go.Scatter(x=df_trend["Date"], y=df_trend["Gross HR"], mode="lines+markers",
                             ...
                             text=df_trend["Snapshot"], hovertemplate="<b>%{text}</b><br>%{x}<br>HR: %{y:.1f} "+unit+"<extra></extra>"))
    fig.add_trace(go.Scatter(x=df_trend["Date"], y=df_trend["Corrected HR"], mode="lines+markers",
                             ...
                             line=dict(width=2, dash="dash"), text=df_trend["Snapshot"]))
    fig.add_trace(go.Scatter(x=df_trend["Date"], y=df_trend["HR Trend"], mode="lines",
                             name=f"Trend ({slope_hr:.2f} {unit}/point)", line=dict(color="gray", dash="dot")))
    fig.update_layout(xaxis_title="Date", yaxis_title=f"Heat Rate ({unit})", height=400,
...
if not mode_data:
    ...
    with st.expander("📋 Mode Aggregation Table"):
        st.dataframe(mode_agg, use_container_width=True)

st.divider()

# ---- Sections 3-6 (Waterfall, Envelope, Filter, Vibration) ----
# These sections are unchanged from the previous correct version.
# For completeness, I include them below (they were already provided).
# If you have them, you can keep them as-is.

# ============================================================
# SECTION 3: HEAT RATE DECOMPOSITION (WATERFALL)
# ============================================================
st.header("🧩 3. Heat Rate Decomposition (Waterfall)")
st.markdown("Breaks down the difference between the selected snapshot and the baseline into contributions from each correction factor.")
...
if st.button("Run Decomposition", key="waterfall_btn"):
    if target_snap == baseline_snap:
        ...
        if "error" in base_res or "error" in target_res:
            ...
            c_names = {"C1": "C1: Inlet Temp", "C2": "C2: Humidity", "C3": "C3: Pressure", "C6": "C6: Inlet Loss", "C7": "C7: Exhaust Loss"}
            waterfall_data = []
            current_hr = base_hr
            ...
            for key in ["C1", "C2", "C3", "C6", "C7"]:
                if key in base_factors and key in target_factors:
                    ...
                    if base_val != 0:
                        ...
                        current_hr += impact
                        waterfall_data.append({"Category": c_names[key], "Value": impact, "Label": f"{impact:+.1f}"})
            ...
            with col3:
                st.metric("Total Delta", f"{delta_total:+.1f} {unit}", delta=f"{delta_total/base_hr*100:+.2f}%")

st.divider()

# ============================================================
# SECTION 4: OPERATING ENVELOPE SCATTER
# ============================================================
...
# ============================================================
# SECTION 5: INLET FILTER HEALTH MONITOR
# ============================================================
...
if not filter_data:
    ...
    fig_filter.add_hline(y=alarm_threshold, line_dash="dash", line_color="red",
                         annotation_text=f"Alarm Threshold: {alarm_threshold} mmwc",
                         annotation_position="bottom right")
    fig_filter.add_hrect(y0=alarm_threshold, y1=df_filter["AFPCS (mmwc)"].max()*1.1,
                         fillcolor="red", opacity=0.1, line_width=0)
    fig_filter.update_layout(xaxis_title="Date", yaxis_title="Inlet Pressure Loss (mmwc)", height=400, hovermode="x unified")
...
# ============================================================
# SECTION 6: COMBUSTION & VIBRATION HEALTH SUMMARY
# ============================================================
