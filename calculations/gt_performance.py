...
class GTPerformance:
    ...
    def _get_demo_value(self, snapshot, column, default=None):
        ...
        if pd.isna(val):
            return default
        return self._safe_float(val, default)

    def calculate(self, snapshot, apply_corrections=True):
        try:
            # ---- Read tags and convert to float ----
            dwatt = self._safe_float(self.db.value("DWATT", snapshot))
            fqg = self._safe_float(self.db.value("FQG", snapshot))
            ctim = self._safe_float(self.db.value("CTIM", snapshot))
            cpd = self._safe_float(self.db.value("CPD", snapshot))
            afpip = self._safe_float(self.db.value("AFPIP", snapshot))
            ...
            if apply_corrections and gross_hr is not None:
                ...
                if afpip:
                    ...
                    c3 = 1.0
                afpcs = self._safe_float(self.db.value("AFPCS", snapshot))
                c6 = rule_engine.evaluate_curve("C6_Inlet_Loss", afpcs, asset_type=self.asset_type) if afpcs else 1.0
                exh_loss = self._safe_float(self.db.value("96EP#1/2#3", snapshot))
